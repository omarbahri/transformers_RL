...
class TabularAgent(RandomAgent):
    def __init__(self, environment, learning_rate=0.1, discount=0.95, exploration_decay=1-1e-4):
        self.environment = environment
        self.action_space = 3
        self.state_space = 20
...
class TabularAgentMonteCarlo(TabularAgent):
    def __init__(self, environment, learning_rate=0.1, discount=0.95, exploration_decay=1-1e-6):
        super().__init__(environment, learning_rate, discount, exploration_decay)
...
class TabularAgentTDLambda(TabularAgent):
    def __init__(self, environment, learning_rate=0.1, discount=0.95, exploration_decay=1-1e-4, n=5):
        super().__init__(environment, learning_rate, discount, exploration_decay)
    ...
    def td_update(self, state, next_state, action, reward):
        ...
        if tau >= 0:
            G = 0
            for i in range(tau + 1, min(tau + self.n + 1,      + 1)):
                G += np.power(self.discount, i - tau - 1) * self.rewards[i]
            if tau + self.n < self.T:
                G += np.power(self.discount, self.n) * self.quality_table[self.states[tau + self.n] + (self.actions[tau + self.n],)]
            # update Q values
            self.quality_table[self.states[tau] + (self.actions[tau],)] += self.learning_rate * (
                        G - self.quality_table[self.states[tau] + (self.actions[tau],)])
...
class TabularAgentOffPolicyTD(TabularAgent):
    def __init__(self, environment, learning_rate=0.1, discount=0.95, exploration_decay=1-1e-3):
        super().__init__(environment, learning_rate, discount, exploration_decay)
...
class TabularAgentDynaQ(TabularAgent):
    def __init__(self, environment, learning_rate=0.1, discount=0.95, exploration_decay=1-1e-4):
        super().__init__(environment, learning_rate)
    ...
    def learn(self, state, next_state, action, reward, done, iteration):
        super().learn(state, next_state, action, reward, done)
